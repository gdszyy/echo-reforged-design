...
物理属性是构筑的基础，它们遵循一套基于“层数消耗”的精确规则，共同决定了弹珠的战场行为。

**A. 核心属性**

- **穿透 (Pierce)**: 拥有N层穿透的弹珠，可以穿透N个敌人。
- **反弹 (Bounce)**: 拥有N层反弹的弹珠，可以在击中敌人后继续反弹N次（撞墙不消耗层数）。
- **散射 (Scatter)**: 弹珠在击中敌人时有概率分裂成多个子弹丸。

**B. 交互规则**

1.  **层数消耗与优先级**:
    - 对于同时拥有穿透和反弹的弹珠，**优先消耗/触发穿透效果**。
    - 每次穿透敌人，消耗一层穿透层数。
    - 穿透层数耗尽后，每次击中敌人，消耗一层反弹层数。
    - 当所有穿透和反弹层数都耗尽后，弹珠消失。

2.  **散射机制**:
    - 每 **1** 层散射属性，提供一个 **50%** 的分裂概率。
    - 这些概率会累加，例如 **2** 层散射会优先合并为 **1** 个 **100%** 的分裂概率，即必定分裂一次。
    - **3.0** 层散射 = **1** 个 **100%** 分裂 + **1** 个 **50%** 分裂。

3.  **属性继承**:
    - 分裂出的子弹丸，会按**固定比例**继承主弹丸的所有属性（包括物理和元素层数）。

4.  **元素应用**:
    - **每一次**弹珠（无论是主弹丸还是分裂出的子弹丸）命中敌人，都会应用其携带的火、电、冰属性效果，并计算温度变化。
...
| :--- | :--- |
| **> 34°C** | 火属性攻击造成额外伤害。 |
| **> 100°C** | 敌人进入**燃烧**状态，每回合受到基于超出部分（温度-100）的持续伤害（DoT）。<br>若敌人在燃烧状态下死亡，会引发**尸爆**，对周围敌人造成基于其最大生命值的伤害，并使其温度上升。 |
